...
- **Действия кнопок**: Выполнение команд, открытие ссылок и копирование в буфер обмена.
...
### Типы ввода (Inputs)
- `text`: Текстовое поле.
- `number-range`: Ползунок (слайдер). Параметры: `min`, `max`, `initial`.

### Действия (Actions)
- `command`: Выполняет команду (например, `say Hello <player>`).
- `url`: Предлагает открыть ссылку.
